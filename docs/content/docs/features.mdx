...
<Backlight>![Features banner](/media/banners/features.png)</Backlight>

For features that do not exist within ckanext-gztr, please request a feature by [adding an issue on GitHub](https://github.com/dathere/ckanext-gztr/issues) or explore **alternative extensions** such as:
...
Commonly, CKAN data catalogs have a search box where users may type their query for datasets they'd like to find. However, this can be cumbersome when it comes to geographic data, when there is no visual method to search and identify relevant regions. The public gazetteer helps resolve this issue by providing an interactive map where users can draw a bounding box to search for datasets that have (simplified) geospatial features which intersect with their drawn bounding box.

<Callout title="The search uses simplified geometries">Since the geospatial search relies on Apache Solr, the bounding box search runs against a **simplified** version of each CKAN dataset's geospatial features indexed in Apache Solr rather than the full geospatial representation.</Callout>
...
ckanext-gztr is being built with the [SpatioTemporal Assets Catalog (STAC) (suite of) specification(s)](https://stacspec.org) in mind regarding its geospatial data organization. This allows for better interoperability such as being able to explore your CKAN instance's geospatial collections through the [STAC Browser](https://browser.moregeo.it/) while also supporting long-term extension usage and customizability.

One of the benefits of this implementation is exposing a STAC API for your geospatial collections. Here is a summary of the endpoints that are made publicly available for your CKAN instance once ckanext-gztr is installed:
...
| `/gztr/stac/collections/{collection_id}/items` | Returns a STAC `ItemCollection` (based on GeoJSON `FeatureCollection`) for a specific STAC Collection as GeoJSON |
| `/gztr/stac/collections/{collection_id}/items/{item_id}` | Returns a STAC `Item` (based on GeoJSON `Feature`) for a specific STAC Item as GeoJSON |

Currently the STAC API `/search` endpoint and validation are not implemented with ckanext-gztr.

**Visit the API reference section in the navigation sidebar/menu** to view the available API endpoints along with using an interactive Postman-like GUI.

{/* TODO: Example users can explore, and GIF "browse your geospatial collections by pointing to your STAC API" */}
